...
SC_MODULE(Hub)
{
    ...
    sc_out<bool>* req_tx;	   
    sc_in<bool>* ack_tx;	  
    sc_in<TBufferCapStatus>* buffer_cap_status_tx;
...
#define STATIC_MAX_CHANNELS 100
      tile2antenna_reservation_table.setSize(STATIC_MAX_CHANNELS);
        ...
        buffer_cap_status_rx = new sc_out<TBufferCapStatus>[num_ports];

        flit_tx = new sc_out<Flit>[num_ports];
        req_tx = new sc_out<bool>[num_ports];
        ack_tx = new sc_in<bool>[num_ports];
        buffer_cap_status_tx = new sc_in<TBufferCapStatus>[num_ports];
