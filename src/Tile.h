...
SC_MODULE(Tile)
{
    ...
    sc_out <TBufferCapStatus> buffer_cap_status_rx[DIRECTIONS];
    ...
    sc_in <TBufferCapStatus> buffer_cap_status_tx[DIRECTIONS];
    ...
    sc_out <TBufferCapStatus> hub_buffer_cap_status_rx;
    ...
    sc_in <TBufferCapStatus> hub_buffer_cap_status_tx;	
    ...
    sc_signal <TBufferCapStatus> buffer_cap_status_rx_local;

    sc_signal <Flit> flit_tx_local;
    sc_signal <bool> req_tx_local;
    sc_signal <bool> ack_tx_local;
    sc_signal <TBufferCapStatus> buffer_cap_status_tx_local;
	...
	for (int i = 0; i < DIRECTIONS; i++) {
	    ...
	    r->req_rx[i] (req_rx[i]);
	    r->ack_rx[i] (ack_rx[i]);
	    r->buffer_cap_status_rx[i](buffer_cap_status_rx[i]);

	    r->flit_tx[i] (flit_tx[i]);
	    r->req_tx[i] (req_tx[i]);
	    r->ack_tx[i] (ack_tx[i]);
	    r->buffer_cap_status_tx[i](buffer_cap_status_tx[i]);
	...
	r->buffer_cap_status_rx[DIRECTION_LOCAL] (buffer_cap_status_tx_local);

	r->flit_tx[DIRECTION_LOCAL] (flit_rx_local);
	r->req_tx[DIRECTION_LOCAL] (req_rx_local);
	r->ack_tx[DIRECTION_LOCAL] (ack_rx_local);
	r->buffer_cap_status_tx[DIRECTION_LOCAL] (buffer_cap_status_rx_local);
	...
	r->buffer_cap_status_rx[DIRECTION_HUB] (hub_buffer_cap_status_rx);

	r->flit_tx[DIRECTION_HUB] (hub_flit_tx);
	r->req_tx[DIRECTION_HUB] (hub_req_tx);
	r->ack_tx[DIRECTION_HUB] (hub_ack_tx);
	r->buffer_cap_status_tx[DIRECTION_HUB] (hub_buffer_cap_status_tx);
	...
	pe->buffer_cap_status_rx(buffer_cap_status_rx_local);
	

	pe->flit_tx(flit_tx_local);
	pe->req_tx(req_tx_local);
	pe->ack_tx(ack_tx_local);
	pe->buffer_cap_status_tx(buffer_cap_status_tx_local);
